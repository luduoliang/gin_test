...
func Login(c *gin.Context){
	if(c.Request.Method == "POST"){
		...
		if(idKeyDTemp == nil){
			c.JSON(200, common.JsonData(false, "", "验证码不正确"))
			return
		}

		idKeyD := idKeyDTemp.(string)
		verifyValue := c.Request.FormValue("captcha")
		...
		if(username == ""){
			c.JSON(200, common.JsonData(false, "", "请输入用户名"))
			return
		}
		if(password == ""){
			c.JSON(200, common.JsonData(false, "", "请输入密码"))
			return
		}
		admin_info := models.GetAdminByUserName(username, password)
		if(admin_info.Id <= 0 || admin_info.Status != 1){
			c.JSON(200, common.JsonData(false, "", "用户名或密码不正确"))
			return
		}

		common.SetSession(c, "admin_info", admin_info)
		c.JSON(200, common.JsonData(true, admin_info, "登录成功"))
	}else{
...
func Captcha(c *gin.Context){
	...
	c.JSON(200, common.JsonData(true, base64stringD, "操作成功"))
